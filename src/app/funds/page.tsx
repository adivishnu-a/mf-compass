...
function resolveSelection(params: Record<string, string | string[] | undefined>) {
  ...
  const category = (categories as readonly string[]).includes(requested) ? requested : categories[0];
...
export async function generateMetadata({ searchParams }: { searchParams: SearchParams }): Promise<Metadata> {
  ...
  return {
    title: isDefault ? "Outperformance Leaderboard" : `${label} funds ranked by outperformance`,
    ...
    alternates: {
      canonical: isDefault ? "/funds" : `/funds?group=${group}&category=${encodeURIComponent(category)}`,
    },
  };
}
...
export default async function FundsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <FundsExplorer group={group} category={category} funds={funds} error={error} />;
}
